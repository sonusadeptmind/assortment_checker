...
function normalizeProductRecord(raw, pid) {
  ...
  const record = {
    ...
    color:        toStrList(r.colors || r.COLOR || r.color || (dump && (dump.colors || dump.COLOR || dump.color))).join(', '),
    sizes:        toStrList(r.sizes || r.RAW_SIZE).join(', '),
    material:     toStrList(r.MATERIAL || r.material || r.FABRIC || r.fabric || (dump && (dump.FABRIC || dump.fabric || dump.MATERIAL || dump.material))).join(', '),
    product_type: firstNonEmpty(r.product_type, r.PRODUCT_TYPE, dump && (dump.product_type || dump.PRODUCT_TYPE)),
    ...
    occasion:     toStrList(r.OCCASION || r.occasion || (dump && (dump.OCCASION || dump.occasion))).join(', '),
