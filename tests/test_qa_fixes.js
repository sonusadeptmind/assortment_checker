 ...
 *   4. OOS progress      — isPidOutOfStock / progressPids / annIsKeywordDone
 *                          exclude out-of-stock products from completion
 */
...
const bundle = [
  ...
  extractFn(appSrc,  'function isPidOutOfStock(',    'app.js'),
  extractFn(appSrc,  'function progressPids(',       'app.js'),
  ...
  extractFn(dataSrc, 'function annIsKeywordDone(',   'annotation/data.js'),
].join('\n\n');
vm.runInContext(bundle, sandbox);

const {
  isPidOutOfStock, progressPids, resolveProductDump,
  recomputeFilteredPids, refreshAfterMarking, annIsKeywordDone, annCountGrades,
} = sandbox;
...
//  4. OOS products excluded from progress / completion
console.log('── issue 4: OOS products excluded from progress ──────────');

const catalog = {
  live1: { liveness: true },
  live2: { liveness: true },
  dead1: { liveness: false },
  dead2: { liveness: false },
  // "unknown" is deliberately absent — absent PIDs default to in-stock
};

assert('liveness:false is OOS',            isPidOutOfStock('dead1', catalog));
assert('liveness:true is in stock',       !isPidOutOfStock('live1', catalog));
assert('absent from catalog → in stock',  !isPidOutOfStock('unknown', catalog));

eq('progressPids drops only the dead ones',
  progressPids(['live1', 'dead1', 'live2', 'dead2', 'unknown'], catalog),
  ['live1', 'live2', 'unknown']);
eq('progressPids on an all-live list is a no-op', progressPids(['live1', 'live2'], catalog), ['live1', 'live2']);
eq('progressPids on an all-dead list is empty',   progressPids(['dead1', 'dead2'], catalog), []);
eq('progressPids tolerates a missing list',       progressPids(undefined, catalog), []);

// annIsKeywordDone reads the module-level productIndex.
sandbox.productIndex = catalog;
sandbox.gradedLabels = {};

const KW_MIXED = { keyword: 'jeans', product_ids: ['live1', 'live2', 'dead1'], re_product_ids: [] };
const KW_ALL_OOS = { keyword: 'clogs', product_ids: ['dead1', 'dead2'], re_product_ids: [] };
const KW_EMPTY = { keyword: 'empty', product_ids: [], re_product_ids: [] };

assert('nothing graded → not done', !annIsKeywordDone(KW_MIXED, 'sonus'));

sandbox.annSetGrade('sonus', 'jeans', 'live1', 1);
assert('half graded → not done', !annIsKeywordDone(KW_MIXED, 'sonus'));

sandbox.annSetGrade('sonus', 'jeans', 'live2', 2);
// This is the reported bug: dead1 can never be graded, and used to keep the
// keyword at 2/3 forever.
assert('every in-stock product graded → done even with an ungraded OOS leftover',
  annIsKeywordDone(KW_MIXED, 'sonus'));

eq('counts run over in-stock products only',
  annCountGrades('sonus', 'jeans', progressPids(KW_MIXED.product_ids)).total, 2);
eq('labeled count matches the in-stock total',
  annCountGrades('sonus', 'jeans', progressPids(KW_MIXED.product_ids)).labeled, 2);

assert('a keyword whose products are all OOS has nothing left to check',
  annIsKeywordDone(KW_ALL_OOS, 'sonus'));
assert('a keyword with no products at all is still not done',
  !annIsKeywordDone(KW_EMPTY, 'sonus'));
assert('no user selected → not done', !annIsKeywordDone(KW_MIXED, ''));

// Grading an OOS product must not push the count past 100%.
sandbox.annSetGrade('sonus', 'jeans', 'dead1', 0);
const c = annCountGrades('sonus', 'jeans', progressPids(KW_MIXED.product_ids));
assert('an OOS grade cannot exceed the denominator', c.labeled <= c.total);
assert('keyword stays done after an OOS product is graded anyway',
  annIsKeywordDone(KW_MIXED, 'sonus'));

// The retailer-level "N keywords checked" bar uses the same exclusion.
assert('updateRetailerProgress filters through progressPids',
  /function updateRetailerProgress\(\)[\s\S]{0,1200}progressPids\(basePids\)/.test(appSrc));
assert('all-OOS keywords count as checked',
  /pids\.length === 0\) \{ doneKws\+\+; return; \}/.test(appSrc));
assert('the sidebar bar scores in-stock products only',
  /const scoredPids = progressPids\(basePids\);/.test(appSrc));
